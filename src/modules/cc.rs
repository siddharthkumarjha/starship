...
macro_rules! impl_common_config {
    ($struct_name:ident) => {
        impl<'a> CommonConfig<'a, ValueError> for $struct_name<'a> {
            fn format(&self) -> &str {
                self.format
            }
            fn version_format(&self) -> &str {
                self.version_format
            }
            fn style(&self) -> &str {
                self.style
...
#[allow(dead_code)]
trait CommonConfig<'a, E>: ModuleConfig<'a, E>
where
    Self: Default + Sync,
    E: SerdeError,
{
...
impl_common_config!(CppConfig);
impl_common_config!(CConfig);
